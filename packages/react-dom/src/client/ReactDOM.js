...
function createRootImpl(
    ...
    options: void | RootOptions, //! 包含一个是否需要调和的属性 `hydrate`
) {
    ...
    //! `createContainer` from  file://react-reconciler/ReactFiberReconciler
    //! root: FiberRootNode
    //! root.cuurent = HostRoot: FiberNode
    //! HostRoot.stateNode = root: FiberRootNode
    const root = createContainer(container, tag, hydrate, hydrationCallbacks);

    //? container['__reactContainere$' + randomKey] = root.current
...
function ReactSyncRoot(
    ...
    //! ReactRoot 会有一个 `_internalRoot` 的属性指向根对象
...
function legacyCreateRootFromDOMContainer(
    ...
    const shouldHydrate =
        forceHydrate || shouldHydrateDueToLegacyHeuristic(container); //! 是否应该调和
    //! First clear any existing content. 首次渲染时如果不需要调和，则会移除所有子节点
    if (!shouldHydrate) {
        let warned = false;
        let rootSibling;
...
function legacyRenderSubtreeIntoContainer(
    ...
    children: ReactNodeList,
    container: DOMContainer, //! render函数的第二个参数，就是根组件的挂载点
    forceHydrate: boolean,  //! 是否强制hydrate
    callback: ?Function,
) {
    if (__DEV__) {
    ...
    if (!root) {
        //! Initial mount (首次挂载)
        //! 可以预见挂载点元素的dom对象总包含一个 `_reactRootContainer` 属性， 它的值是 `ReactRoot` 类型，ReactSyncRoot
        ...
        //! Initial mount should not be batched. (初始安装不应批处理)
        unbatchedUpdates(() => {
            //! children = <App />
