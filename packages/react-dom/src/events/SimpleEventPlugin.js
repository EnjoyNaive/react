...
const eventTuples: Array<EventTuple> = [
    // Discrete events
    [DOMTopLevelEventTypes.TOP_BLUR, 'blur', DiscreteEvent], //! 0
    ...
    [DOMTopLevelEventTypes.TOP_DRAG, 'drag', UserBlockingEvent], //! 1
    ...
    // Continuous events
    [DOMTopLevelEventTypes.TOP_ABORT, 'abort', ContinuousEvent], //! 2
...
for (let i = 0; i < eventTuples.length; i++) {
    ...
    const topEvent = eventTuple[0]; //! 事件名(有前缀)
    const event = eventTuple[1]; //! 事件名(标准名)
    const eventPriority = eventTuple[2]; //! 事件优先级 0 ,1 ,2
