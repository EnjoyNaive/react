...
function getContextForSubtree(
    ...
    const fiber = getInstance(parentComponent); //! parentComponent.__reactInternalFiber
    const parentContext = findCurrentUnmaskedContext(fiber);

    if (fiber.tag === ClassComponent) {
        const Component = fiber.type;
        if (isLegacyContextProvider(Component)) {
            return processChildContext(fiber, Component, parentContext); //! 会合并 `parentContext` 和 `childContext(包含getChildContext 和 通过 childContextTypes 检查, 不然会报错)`
        }
    }
...
): OpaqueRoot { //! OpaqueRoot = FiberRootNode
    return createFiberRoot(containerInfo, tag, hydrate, hydrationCallbacks); //! 返回的是一个 `FiberRootNode`
}
